...
class MatchingEngine:
    ...
    # Generic customer-money-in receipt types (a plain payment, a gateway
    # collection, or an ad-hoc/unspecified receipt) may only settle invoice
    # types that represent an actual outstanding receivable. Without this,
    # fuzzy customer-name + amount matching can accidentally pair an
    # ad-hoc receipt against something like a Security Deposit Invoice or a
    # Write-Off, which is never a valid settlement.
    GENERIC_SETTLEABLE_TYPES = {"Standard Invoice", "Advance Adjusted Invoice", "Disputed Invoice"}
    ...
    RECEIPT_TYPE_INVOICE_TYPES = {
        ...
        "customer_payment": GENERIC_SETTLEABLE_TYPES,
        "gateway": GENERIC_SETTLEABLE_TYPES,
        "ad_hoc": GENERIC_SETTLEABLE_TYPES,
        "cheque_deposit": GENERIC_SETTLEABLE_TYPES,
        "other_receipt": GENERIC_SETTLEABLE_TYPES,
    }
    ...
    def run(self):
        ...
        result["emi_schedule_df"] = self._build_emi_schedule(invoices, matched_df, possible_df)
        return result
    ...
    def _prepare_invoices(self):
        ...
        invoices["_invoice_date"] = self._parse_date(invoices.get("Invoice Date"))

        date_column = self._first_existing_column(invoices, ["Due Date", "Invoice Date"])
        invoices["_match_date"] = (
            self._parse_date(invoices[date_column]) if date_column else pd.NaT
        )
    ...
    def _parse_date(value):
        ...
        if isinstance(parsed, pd.Series):
            fallback_mask = parsed.isna() & pd.notna(value)
            if fallback_mask.any():
                parsed.loc[fallback_mask] = pd.to_datetime(
                    value[fallback_mask], errors="coerce"
                )
        return parsed
    ...
    def _bank_date(self, row):
        for column in ["Txn Date", "Transaction Date", "Value Date", "Date"]:
            if column in row.index:
                return self._parse_date(row.get(column))
        return pd.NaT
    ...
    def _build_emi_schedule(self, invoices, matched_df, possible_df):
        emi_invoices = invoices[invoices["_invoice_type_norm"] == "EMI Purchase"].copy()
        if emi_invoices.empty:
            return pd.DataFrame(columns=[
                "Invoice No.", "Customer Name", "Total Invoice Amt", "Total EMIs",
                "EMI No.", "Due Date", "EMI Amt (INR)", "Status", "Payment Date", "Remarks",
            ])

        emi_invoices["_due_date"] = self._parse_date(emi_invoices.get("Due Date"))
        emi_invoices["_group_key"] = (
            emi_invoices["Customer Name"].astype(str) + "||"
            + emi_invoices["Invoice Date"].astype(str)
        )

        # payment lookup: invoice no. -> (status, payment date)
        payment_lookup = {}
        for df, status_label in [(matched_df, "Paid"), (possible_df, "Paid (Review Required)")]:
            if df.empty:
                continue
            date_col = self._first_existing_column(df, ["Txn Date", "Transaction Date", "Value Date", "Date"])
            for _, row in df.iterrows():
                invoice_no = row.get("Matched Invoice")
                if pd.isna(invoice_no):
                    continue
                payment_date = row.get(date_col) if date_col else None
                payment_lookup[invoice_no] = (status_label, payment_date)

        today = pd.Timestamp.today().normalize()
        rows = []

        for _, group in emi_invoices.groupby("_group_key", sort=False):
            group = group.sort_values("_due_date")
            total_emis = len(group)
            total_amt = group["_amount"].iloc[0]
            emi_amt = self._round_amount(total_amt / total_emis) if total_emis else 0

            for position, (_, invoice_row) in enumerate(group.iterrows(), start=1):
                invoice_no = invoice_row["_invoice_no_norm"]
                customer = invoice_row.get("Customer Name", "")
                due_date = invoice_row["_due_date"]

                if invoice_no in payment_lookup:
                    status, payment_date = payment_lookup[invoice_no]
                elif pd.notna(due_date) and due_date < today:
                    status, payment_date = "Overdue", None
                elif pd.notna(due_date) and due_date <= today + pd.Timedelta(days=7):
                    status, payment_date = "Due Soon", None
                else:
                    status, payment_date = "Future", None

                rows.append({
                    "Invoice No.": invoice_no,
                    "Customer Name": customer,
                    "Total Invoice Amt": total_amt,
                    "Total EMIs": total_emis,
                    "EMI No.": position,
                    "Due Date": due_date.date() if pd.notna(due_date) else None,
                    "EMI Amt (INR)": emi_amt,
                    "Status": status,
                    "Payment Date": (
                        payment_date.date() if isinstance(payment_date, pd.Timestamp) else payment_date
                    ),
                    "Remarks": f"EMI {position} of {total_emis} – {customer}",
                })
